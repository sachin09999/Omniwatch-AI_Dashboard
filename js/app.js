...
class OmniVisionApp {
    ...
    updateConnectionStatus() {
        ...
        if (api.lastResponseStatus === 'live') {
            ...
            label.textContent = 'LIVE: 10.10.12.52:8009';
    ...
    openInspectorModal(item) {
        ...
        const detection = item.detections?.[0] || {};
        const meta = detection.metadata || {};
        const plateText = meta.plate_text || detection.class_name || '2001';
        const plateOrigin = meta.plate_origin || 'UAE';
        const confPercent = Math.round((detection.confidence || 0) * 1000) / 10;

        // Set metadata fields
        const srIdEl = document.getElementById('modalSrId');
        if (srIdEl) srIdEl.textContent = `ID: #${item.sr_id}`;
        
        const plateTextEl = document.getElementById('modalPlateText');
        if (plateTextEl) plateTextEl.textContent = plateText;

        const plateOriginEl = document.getElementById('modalPlateOrigin');
        if (plateOriginEl) plateOriginEl.textContent = plateOrigin;

        const confEl = document.getElementById('modalConfidence');
        if (confEl) confEl.textContent = `${confPercent > 0 ? confPercent + '%' : '99.85%'}`;

        const camEl = document.getElementById('modalCamera');
        if (camEl) camEl.textContent = item.camera_name || 'Terrace-Cam-23';

        const zoneEl = document.getElementById('modalZone');
        if (zoneEl) zoneEl.textContent = item.zone_name || 'Zone 02';

        const timeEl = document.getElementById('modalTimestamp');
        if (timeEl) timeEl.textContent = this.formatTimestamp(item.detected_at);

        const cropImg = document.getElementById('modalCropImg');
        const thumbUrl = api.resolveMediaUrl(item.thumbnail_url);
        if (cropImg && thumbUrl) {
            cropImg.src = thumbUrl;
        }

        const cleanImg = document.getElementById('modalCleanImg');
        const photoUrl = api.resolveMediaUrl(item.photo_url || item.thumbnail_url);
        if (cleanImg && photoUrl) {
            cleanImg.src = photoUrl;
        }

        const videoPlayer = document.getElementById('modalVideoPlayer');
        if (videoPlayer) {
            videoPlayer.src = api.resolveMediaUrl(item.video_url || item.photo_url?.replace('/photo', '/video'));
            videoPlayer.poster = photoUrl;
        }

        modal.classList.add('active');

        // Slide switcher logic
        const slideNames = ['AI Snapshot Overlay', 'High-Res Camera Photo', 'Video Stream Recording'];
        const setSlide = (idx) => {
            this.state.currentSlide = (idx + 3) % 3;
            ...
            if (slide0) slide0.style.display = current === 0 ? 'flex' : 'none';
            if (slide1) slide1.style.display = current === 1 ? 'flex' : 'none';
            if (slide2) slide2.style.display = current === 2 ? 'flex' : 'none';

            if (current !== 2 && videoPlayer) {
                videoPlayer.pause();
            }

            const stepEl = document.getElementById('modalSlideStep');
            if (stepEl) stepEl.textContent = `${current + 1} of 3`;

            const nameEl = document.getElementById('modalSlideName');
            if (nameEl) nameEl.textContent = slideNames[current];

            const dots = document.querySelectorAll('#modalCarouselDots .anpr-dot');
            dots.forEach((dot, dIdx) => {
                if (dIdx === current) dot.classList.add('active');
        ...
        if (prevArrow) {
            prevArrow.onclick = (e) => {
                e.stopPropagation();
                setSlide(this.state.currentSlide - 1);
            };
        }
        ...
        dots.forEach((dot) => {
            dot.onclick = (e) => {
                e.stopPropagation();
                const targetIdx = parseInt(dot.getAttribute('data-slide') || '0', 10);
                setSlide(targetIdx);
            };
        });

        // Fullscreen toggle
        const viewport = document.getElementById('modalMediaViewport');
        const toggleFullscreen = () => {
            if (!viewport) return;
            if (!document.fullscreenElement) {
                if (viewport.requestFullscreen) viewport.requestFullscreen().catch(() => {});
                else viewport.classList.toggle('fullscreen');
            } else {
        ...
        const fsBtn = document.getElementById('modalBtnFullscreen');
        if (fsBtn) fsBtn.onclick = (e) => { e.stopPropagation(); toggleFullscreen(); };

        const fsOverlayBtn = document.getElementById('modalFullscreenOverlayBtn');
        if (fsOverlayBtn) fsOverlayBtn.onclick = (e) => { e.stopPropagation(); toggleFullscreen(); };
        ...
        if (cleanImg) {
            cleanImg.onclick = () => toggleFullscreen();
        }
        ...
        if (downloadBtn) {
            downloadBtn.onclick = (e) => {
                e.stopPropagation();
                if (canvas && this.state.currentSlide === 0) {
                    ...
                    const link = document.createElement('a');
                    link.download = `detection_${plateText}_${item.sr_id}.jpg`;
                    link.href = photoUrl;
                    link.target = '_blank';
                    link.click();
                }
